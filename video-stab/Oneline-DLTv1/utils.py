...
def transformer(U, theta, out_size, **kwargs):
    ...
    空间变压器层
    实现如[1]_所述的空间转换层。
    基于[2]_并由David Dao为Tensorflow编辑。
    参数
    ----------
    U : float
    ...
    U:浮动
    卷积网络的输出应该有
    形状[num_batch, height, width, num_channels]。
    θ:浮动
    的输出
    本地化网络应该是[num_batch, 6]。
    Out_size:两个整数元组
    网络输出的大小(高、宽)
    ----------
    .. [1]  Spatial Transformer Networks空间变压器网络
    ...
    [0。1。, 0]])。
    身份= identity.flatten ()
    θ= tf.Variable (initial_value =身份)
    """
