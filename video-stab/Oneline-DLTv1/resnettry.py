...
from main import mycompute_mask
import numpy as np
...
class ResNet(nn.Module):
    ...
    def forward(self, org_imges, input_tesnors, h4p, patch_indices, org_imgs):
        ...
        mask_I1_full = org_imgs[:, :1, ...]
        mask_I2_full = org_imgs[:, 1:, ...]


        pre_I1_full = self.genMask(org_imges[:, :1, ...])
        pre_I2_full = self.genMask(org_imges[:, 1:, ...])
        ...
        pre_I1 = getPatchFromFullimg(patch_size_h, patch_size_w, patch_indices, batch_indices_tensor, pre_I1_full)
        pre_I2 = getPatchFromFullimg(patch_size_h, patch_size_w, patch_indices, batch_indices_tensor, pre_I2_full)


        mask_I1 = normMask(mask_I1)
        mask_I2 = normMask(mask_I2)

        pre_I1 = normMask(pre_I1)
        pre_I2 = normMask(pre_I2)

        mask_I1 = torch.mul(pre_I1, mask_I1)
        mask_I2 = torch.mul(pre_I2, mask_I2)
